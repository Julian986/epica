...
function buildServiceItemsForInput(input: CreateReservationInput): {
  ...
  const lacios = uniqueIds.filter(isLacioTreatmentId);
  const comps = uniqueIds.filter(isComplementarioTreatmentId);
  if (lacios.length > 1 || comps.length > 1) return null;
  if (lacios.length + comps.length !== uniqueIds.length) return null;
